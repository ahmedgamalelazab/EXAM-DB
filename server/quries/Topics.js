...
module.exports.generateTopicReportById = async function (topic_id) {
  return new Promise(async (resolve, reject) => {
    if (MSSQLConnection.pool.connected) {
      //else if its connected
      try {
        await generateTopicReportProc();

        //if it's created well

        await MSSQLConnection.pool
          .request()
          .input('topic_id', topic_id)
          .execute(
            DBProcedureDictionary.generateTopicReportProc,
      ...
      reject(new Error(`u need to connect to the db to call get topic query`));
    }
  });
};
